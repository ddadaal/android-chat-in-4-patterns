...
import nju.androidchat.client.mvvm5.model.ClientMessageObservable;
...
@Log
public class Mvvm5ViewModel extends BaseObservable implements MessageListener, Counter {
    ...
    @Bindable
    @Getter
    private int messageToReadNum = 0;

    @Getter
    private UiOperator uiOperator;

    public void setMessageToReadNum(int messageToReadNum) {
        this.messageToReadNum = messageToReadNum;
        notifyPropertyChanged(BR.messageToReadNum);
    }
    ...
    public Mvvm5ViewModel(UiOperator uiOperator) {
...
//            因为要统计未读消息，每次添加都滚动到底部的话就全部都是已读消息了
